...
Todas las dimensiones se declaran en una escala 1-5 y se normalizan al rango [0, 1]
dividiendo por 5. La parametrización es explícita y forma parte de la auditabilidad
del modelo: cualquier valor del ContextFactor puede trazarse hasta estas tablas.

**Exposición.** El grado de exposición se declara según la accesibilidad del activo:

| Nivel | Descripción             | Valor | Norm. |
|-------|-------------------------|-------|-------|
| 5     | Internet-facing         | 5     | 1.0   |
| 4     | DMZ / acceso controlado | 4     | 0.8   |
| 3     | Red corporativa         | 3     | 0.6   |
| 2     | Interno restringido     | 2     | 0.4   |
| 1     | Aislado                 | 1     | 0.2   |

En el inventario, los valores textuales se corresponden con esta escala:
`internet` -> 5 (1.0), `internal` -> 3 (0.6), `restricted_internal` -> 2 (0.4).

**Sensibilidad arquitectónica del módulo.** Refleja la importancia de un componente
por el papel que desempeña en la arquitectura y los sistemas que dependen de él:

| Módulo | Descripción | Valor | Norm. |
|--------|-------------|-------|-------|
| B01 | Perímetro web | 4 | 0.8 |
| B02.1 | Aplicación (secundaria) | 3 | 0.6 |
| B02.2 | Aplicación (transaccional/crítica) | 4 | 0.8 |
| B03.1 | BB.DD. relacionales (auxiliar) | 4 | 0.8 |
| B03.2 | BB.DD. relacionales (datos críticos) | 5 | 1.0 |
| B04 | Identidad corporativa | 5 | 1.0 |
| B05 | Conectividad segura | 5 | 1.0 |
| B06.1 | SDN / microsegmentación (complementaria) | 4 | 0.8 |
| B06.2 | SDN / microsegmentación (principal) | 5 | 1.0 |
| B07 | Cloud compute | 4 | 0.8 |
| B08.1 | Cloud storage (moderado) | 4 | 0.8 |
| B08.2 | Cloud storage (crítico) | 5 | 1.0 |
| B09 | Cloud identidad | 5 | 1.0 |
| B10.1 | Contenedores (acotados) | 4 | 0.8 |
| B10.2 | Contenedores (productivos) | 5 | 1.0 |
| B11 | OT / industrial | 4 | 0.8 |
| B12.1 | Endpoint estándar | 3 | 0.6 |
| B12.2 | Endpoint privilegiado | 4 | 0.8 |
| B12.3 | Endpoint administrativo | 5 | 1.0 |

**Criticidad del módulo.** Valor por defecto de criticidad según el impacto de la
indisponibilidad del módulo. Puede sustituirse por un override declarado (factor H):

| Módulo | Descripción | Nivel | Valor | Norm. |
|--------|-------------|-------|-------|-------|
| B01 | Perímetro web | Alto | 4 | 0.8 |
| B02.1 | Aplicación (secundaria) | Medio | 3 | 0.6 |
| B02.2 | Aplicación (transaccional/crítica) | Alto | 4 | 0.8 |
| B03.1 | BB.DD. relacionales (soporte) | Medio | 3 | 0.6 |
| B03.2 | BB.DD. relacionales (críticas) | Alto | 4 | 0.8 |
| B04 | Identidad corporativa | Crítico | 5 | 1.0 |
| B05 | Conectividad segura | Alto | 4 | 0.8 |
| B06.1 | SDN / microsegmentación | Medio | 3 | 0.6 |
| B07 | Cloud compute | Medio | 3 | 0.6 |
| B08.1 | Cloud storage (moderado) | Medio | 3 | 0.6 |
| B08.2 | Cloud storage (crítico) | Alto | 4 | 0.8 |
| B09 | Cloud identidad | Crítico | 5 | 1.0 |
| B10.1 | Contenedores | Medio | 3 | 0.6 |
| B11 | OT / industrial | Medio | 3 | 0.6 |
| B12.1 | Endpoint estándar | Bajo | 2 | 0.4 |
| B12.2 | Endpoint privilegiado | Medio | 3 | 0.6 |
| B12.3 | Endpoint administrativo | Alto | 4 | 0.8 |

Los niveles se corresponden con Bajo -> 2 (0.4), Medio -> 3 (0.6), Alto -> 4 (0.8),
Crítico -> 5 (1.0). El override declarado emplea la misma escala (low -> 2,
medium -> 3, high -> 4, critical -> 5).

**Blast radius.** El radio de impacto de un módulo se calcula sobre una matriz de
alcance declarada, según la fracción de módulos a los que puede propagarse un
compromiso:

```
BlastRadius_norm = ( Σ módulos de alcance directo × 1.0
                   + Σ módulos de alcance a un salto × 0.5 ) / N
```
